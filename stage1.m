...
output_root = sprintf('/home/zeng/data/datasets/saliency_Dataset/%s/Ours', dataset);
...
for i_img = 1:numel(img_name_list)
    ...
    img_obj = (img_obj-min(img_obj(:))) / (max(img_obj(:))-min(img_obj(:)));
    img_lab = colorspace('Lab<-', img);
    sal_map = 1;
    rlbmap = 1;
    [m, n, ~] = size(img_lab);
    img_lab = reshape(img_lab, m*n, 3);
    img_lab = (img_lab-min(img_lab)) ./ (max(img_lab)-min(img_lab));
...
%         Wh(Adj==1) = Whd(Adj==1);
        ...
        sal = superpixel_saliency(Wh,bd,sp_pos, sp_obj,2.1e-7,9e-8,0.007, sp_num); 
